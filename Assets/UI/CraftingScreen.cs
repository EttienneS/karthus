...
public class CraftingScreen : MonoBehaviour
{
    ...
    public void Show(Structure craftSource)
    {
        ...
        //foreach (Craft task in FactionManager.Factions[FactionConstants.Player].GetTaskByOriginator(_craftSource.Id))
        //{
    ...
    public void Make(int amount)
    {
        for (var i = 0; i < amount; i++)
        {
            ...
            //FactionManager.Factions[FactionConstants.Player].AddTask(task, _craftSource.Id);
    ...
    private void Scale()
    {
        ...
        QueuePanel.GetComponent<RectTransform>().SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, FactionController.PlayerFaction.GetTaskByOriginator(_craftSource.Id).Count() * 70f);
