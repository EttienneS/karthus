...
public class MapGenerator
{
    ...
    public List<CellData> GetLine(CellData a, CellData b)
    {
        ...
        var x = a.Coordinates.X;
        var y = a.Coordinates.Y;
        ...
        {
            line.Add(Game.MapGrid.GetCellAtCoordinate(new Coordinates(x, y)));
            numerator += shortest;
    ...
    public List<CellData> GetRectangle(CellData cell, int width, int height)
    {
        ...
        var fromX = Math.Min(cell.Coordinates.X, cell.Coordinates.X + width);
        var toX = Math.Max(cell.Coordinates.X, cell.Coordinates.X + width);

        var fromY = Math.Min(cell.Coordinates.Y, cell.Coordinates.Y + height);
        var toY = Math.Max(cell.Coordinates.Y, cell.Coordinates.Y + height);
    ...
    public List<CellData> HollowSquare(List<CellData> square)
    {
        ...
        var src = Game.MapGrid.GetCellAtCoordinate(new Coordinates(minMax.minx + 1, minMax.miny + 1));
    ...
    {
        var mineX = Clamp((int)(center.Coordinates.X + (radius * Math.Cos(angle))), 0, Game.MapGrid.Width);
        var mineY = Clamp((int)(center.Coordinates.Y + (radius * Math.Sin(angle))), 0, Game.MapGrid.Height);

        return Game.MapGrid.GetCellAtCoordinate(new Coordinates(mineX, mineY));
    }

    internal float GetDegreesBetweenPoints(CellData point1, CellData point2)
    {
        var deltaX = point1.Coordinates.X - point2.Coordinates.X;
        var deltaY = point1.Coordinates.Y - point2.Coordinates.Y;
    ...
    internal CellData GetPointAtDistanceOnAngle(CellData origin, int distance, float angle)
    {
        ...
        var tX = origin.Coordinates.X;
        var tY = origin.Coordinates.Y;

        if (angle != 0 && angle != 180)
        {
            tY = (int)((Math.Sin(-radians) * distance) + origin.Coordinates.Y);
        }

        if (angle != 90 && angle != 270)
        {
            tX = (int)((Math.Cos(radians) * distance) + origin.Coordinates.X);
        }

        // add 1 to offset rounding errors
        return Game.MapGrid.GetCellAtCoordinate(new Coordinates(tX, tY));
    }

    internal CellData GetRandomRadian(CellData origin, int radius)
    {
        var angle = Random.Range(0, 360);
        var mineX = Clamp((int)(origin.Coordinates.X + (radius * Math.Cos(angle))), 0, Game.MapGrid.Width);
        var mineY = Clamp((int)(origin.Coordinates.Y + (radius * Math.Sin(angle))), 0, Game.MapGrid.Height);
        return Game.MapGrid.GetCellAtCoordinate(new Coordinates(mineX, mineY));
    }
        ...
        foreach (var street in Streets)
        {
            for (int cellIndex = 0; cellIndex < street.Count - 1; cellIndex++)
            {
                ...
                var neighbours = new List<CellData>
                    {
                        cell,
                        Game.MapGrid.GetCellAtCoordinate(new Coordinates(cell.Coordinates.X + 1, cell.Coordinates.Y)),
                        Game.MapGrid.GetCellAtCoordinate(new Coordinates(cell.Coordinates.X - 1, cell.Coordinates.Y)),
                        Game.MapGrid.GetCellAtCoordinate(new Coordinates(cell.Coordinates.X, cell.Coordinates.Y + 1)),
                        Game.MapGrid.GetCellAtCoordinate(new Coordinates(cell.Coordinates.X, cell.Coordinates.Y - 1)),
                    };
    ...
    private List<CellData> GetCorners(List<CellData> square)
    {
        ...
        return new List<CellData>
            {
                Game.MapGrid.GetCellAtCoordinate(new Coordinates(minMax.minx, minMax.miny)),
                Game.MapGrid.GetCellAtCoordinate(new Coordinates(minMax.minx, minMax.maxy)),
                Game.MapGrid.GetCellAtCoordinate(new Coordinates(minMax.maxx, minMax.miny)),
                Game.MapGrid.GetCellAtCoordinate(new Coordinates(minMax.maxx, minMax.maxy))
            };
    }
    ...
    private List<CellData> GetPossibleDoors(List<CellData> building)
    {
        ...
        {
            if (corners.Any(c => c.Coordinates.X == cell.Coordinates.X && c.Coordinates.Y == cell.Coordinates.Y))
            {
                continue;
            }
    ...
    private List<CellData> Grow(List<CellData> cells, int size)
    {
        ...
        foreach (var cell in cells)
        {
            for (int x = -size; x <= size; x++)
            {
                ...
                {
                    var tx = cell.Coordinates.X + x;
                    var ty = cell.Coordinates.Y + y;

                    if (group.Any(g => g.Coordinates.X == tx && g.Coordinates.Y == ty))
                    {
                        continue;
                    }
                    ...
                    {
                        group.Add(Game.MapGrid.GetCellAtCoordinate(new Coordinates(tx, ty)));
                    }
                }
            }
        }
